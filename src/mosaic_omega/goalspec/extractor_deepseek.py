...
except ImportError:  # 让 rule 模式不受 openai 包缺失影响
...
def extract_with_deepseek(user_text: str) -> Dict[str, Any]:
    ...
    if OpenAI is None:
        raise RuntimeError("缺少 openai Python SDK，请先运行：py -m pip install -r requirements.txt")

    api_key = os.getenv("DEEPSEEK_API_KEY")
    if not api_key:
        raise RuntimeError(
            "没有检测到 DEEPSEEK_API_KEY。请先在 PowerShell 中运行："
    ...
    raise RuntimeError(f"DeepSeek 抽取失败：{last_error}")
