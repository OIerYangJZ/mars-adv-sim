...
class Orchestrator:
    ...
    def run_once(self, run_id: str) -> list[str]:
        ...
        for assignment in assignments:
            ...
            try:
                ...
                if agent is None:
                    raise RuntimeError(
                        f"assigned agent has no execution adapter: {assignment.agent_id}"
                    )
                ...
                if passed:
                    self.events.transition(
                        ...
                        actor_id="orchestrator",
                        trace_id=trace_id,
                        reason="verification predicates passed",
                    )
                    ...
                    self._recover_or_fail(
                        ...
                        error_class=error_class,
                        reason=combined.error or "verification failed",
                    )
                ...
                if current and current.state in {TaskState.RUNNING, TaskState.VERIFYING}:
                    failed_result = combined or ExecutionResult(
                        call_id=f"failure_{uuid.uuid4().hex}",
                        success=False,
                        error=f"{type(exc).__name__}: {exc}",
                        error_class=error_class,
                    )
    ...
    def _combine(results: list[ExecutionResult]) -> ExecutionResult:
        ...
        return ExecutionResult(
            ...
            metadata={"tool_call_count": len(results)},
            error_class=error_class,
        )
