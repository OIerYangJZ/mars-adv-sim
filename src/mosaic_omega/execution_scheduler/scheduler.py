...
class Scheduler:
    ...
    def _candidates(self, task: TaskNodeView) -> dict[str, Assignment]:
        groups = [
    ...
    def _ortools(self, tasks: list[TaskNodeView]) -> list[Assignment]:
        ...
        capacities = self._capacities()
        candidates = {task.task_id: self._candidates(task) for task in tasks}
        ...
        if sum(capacities.get(device_id, 0) for device_id in device_ids) < len(schedulable):
            raise NoFeasibleAssignment("insufficient device capacity")
        source = 0
        ...
        flow.set_node_supply(source, len(schedulable))
        flow.set_node_supply(sink, -len(schedulable))
        status = flow.solve()
        ...
        for arc in range(flow.num_arcs()):
            ...
            if tail in reverse_tasks and head in reverse_devices:
                ...
                results.append(self._policy(candidates[task_id][device_id], "ortools"))
        return results
