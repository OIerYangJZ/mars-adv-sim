...
from mosaic_omega.integration.planner_runtime_bridge import dag_to_task_specs
from mosaic_omega.goal_planner.todag.engine import ToDAGEngine
from mosaic_omega.goal_planner.todag.models import INPUT_FIELDS, DAGNode, LongTaskInput
from mosaic_omega.goal_planner.todag.graph import critical_path, descendants, topological_sort
from mosaic_omega.goal_planner.todag.web import create_server
...
class ToDAGEngineTests(unittest.TestCase):
    def test_build_has_typed_taskgraph_and_coordinator_compatibility(self) -> None:
        ...
        specs = dag_to_task_specs(engine.coordinator_plan())
        self.assertEqual(len(specs), len(result["nodes"]))
    ...
    def test_enriched_goalspec_drives_dependencies_predicates_and_placement(self) -> None:
        ...
        plan_node = next(item for item in engine.coordinator_plan() if item["task_id"] == patch["task_id"])
    ...
    def test_update_recomputes_only_changed_branch_and_final_gate(self) -> None:
        ...
        engine.set_node_result(requirement_id, {"ok": True})
        engine.set_node_result(first_work_id, {"artifact": "keep"})
        engine.set_node_result(first_verify["task_id"], {"verified": True})
        ...
        after_recompute = engine.set_node_result(second_work_id, {"artifact": "new"})
    ...
    def test_missing_acceptance_conditions_blocks_coordinator_export(self) -> None:
        raw = sample_input()
...
class ToDAGWebTests(unittest.TestCase):
    def test_build_patch_and_goalspec_update_api(self) -> None:
        ...
        try:
            ...
            request = Request(f"{base}/api/build", data=body, method="POST", headers={"Content-Type": "application/json"})
            with urlopen(request) as response:
                built = json.loads(response.read())
            self.assertEqual(built["status"], "ready")
            work_id = nodes_by_type(built, "work")[0]["task_id"]
            patch_body = json.dumps({"description": "updated method"}).encode("utf-8")
            request = Request(
                f"{base}/api/nodes/{work_id}",
                data=patch_body,
                method="PATCH",
            ...
            self.assertEqual(updated["change_set"]["changed_node_ids"], [work_id])

            changed = sample_input()
            changed["acceptance_conditions"].append("show reproducibility")
            request = Request(
                f"{base}/api/specification",
                data=json.dumps(changed).encode("utf-8"),
                method="PUT",
                headers={"Content-Type": "application/json"},
            )
            with urlopen(request) as response:
                replanned = json.loads(response.read())
            self.assertGreater(replanned["revision"], updated["revision"])
            self.assertTrue(replanned["change_set"]["added_node_ids"])
        finally:
            server.shutdown()
            server.server_close()
