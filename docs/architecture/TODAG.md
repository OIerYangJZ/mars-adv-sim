...
`goal_planner.todag` 只负责 **GoalSpec → TaskGraph → execution plan**。它不维护运行期事实状态；任务状态、事件、Evidence 与 Replay 的唯一事实源是 `execution_scheduler.EventStore`。
...
```bash
PYTHONPATH=src python -m mosaic_omega.goal_planner.todag build goalspec.json \
  -o taskgraph.json \
